...
try {
    ...
    $select_columns = ["id", "title", "content", "sent_by", "created_at", "expiry_date"];
...
function getAnnouncementStatus($created_at, $is_active = null, $expiry_date = null) {
                            ...
                            <?php else: ?>
                                <?php foreach ($announcements as $announcement): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <div class="datetime-info expiry-date">
                                                <?php echo formatDateTime($announcement['expiry_date'] ?? ''); ?>
                                            </div>
                                        </td>
                                        <td>
                                            <?php 
    ...
    <script>
        ...
        function viewAnnouncement(id) {
            ...
            rows.forEach(row => {
                ...
                if (firstCell) {
                    // Since ID column is removed, we need to find the row by matching data
                    // We'll use the title to identify the row (this is a simplified approach)
                    const cells = row.querySelectorAll('td');
                    if (cells.length > 0) {
                        // We'll assume the first button in action column has the correct ID
                        const viewButton = cells[6].querySelector('.btn-view');
                        if (viewButton && viewButton.getAttribute('onclick').includes(id)) {
                            announcementData = {
                                id: id,
                                title: cells[0].textContent.trim(),
                                recipient: cells[1].textContent.trim(),
                                type: cells[2].textContent.trim(),
                                dateSent: cells[3].textContent.trim(),
                                expiryDate: cells[4].textContent.trim(),
            ...
            if (announcementData) {
                ...
                const modalContent = `
                    ...
                    <div class="announcement-detail">
                        ...
                        <div class="datetime-info">${announcementData.dateSent}</div>
                    </div>
                    <div class="announcement-detail">
                        <label>Expiry Date & Time:</label>
                        <div class="datetime-info expiry-date">${announcementData.expiryDate}</div>
                    </div>
                    <div class="announcement-detail">
                        <label>Status:</label>
